...
void main()
{
    ...
    float smoothFresnel =  clamp(dot(viewDir, normalize(smoothNormal  )), 0,1);
   ...
   vec4 heartColor = mix(vec4(143, 239, 250, 255)/255.0, vec4(1,0,0,1), fov);
   gradient= mix(gradient, heartColor,  smoothFresnel -.2 + heartbeat *.2);
   gradient = mix(gradient,  vec4(241, 122, 238, 255)/255.0, fresnel);
